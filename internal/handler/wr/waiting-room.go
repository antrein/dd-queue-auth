...
import (
	...
	guard "antrein/dd-queue-auth/application/middleware"
	"antrein/dd-queue-auth/internal/utils"
	"antrein/dd-queue-auth/model/config"
	"antrein/dd-queue-auth/model/dto"
	"antrein/dd-queue-auth/model/entity"
	"context"
	"fmt"
	"net/http"
	"time"
...
func (h *Handler) RegisterHandler(app *http.ServeMux) {
	app.HandleFunc("/dd/queue/register", guard.DefaultGuard(h.RegisterQueue))
	app.HandleFunc("/dd/queue/wr", guard.AuthGuard(h.cfg, h.UserQueue))
}
